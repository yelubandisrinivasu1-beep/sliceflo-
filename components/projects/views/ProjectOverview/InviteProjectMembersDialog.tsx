...
interface InviteProjectMembersDialogProps {
  open: boolean;
  onClose: () => void;
  projectId: string;
  projectName: string;
  existingMemberIds: string[]; // userId[]
  onMembersUpdate?: () => void;
}

export default function InviteProjectMembersDialog({
  open,
  onClose,
  projectId,
  projectName,
  existingMemberIds = [],
  onMembersUpdate,
}: InviteProjectMembersDialogProps) {
  ...
  const { addMembersToProject } = useProjectsStore();
  ...
  const allUsers = useMemo(() => {
    return workspaceMembers.map((u: any) => {
      const isAlready = existingMemberIds.includes(u.userId);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast('success', { title: 'Members added successfully!' });
      onMembersUpdate?.();
      onClose();
    } catch (error) {
      toast('error', { title: 'Failed to add members' });
    } finally {
      setIsLoading(false);
    }
  };
              ...
              {rows.map((row, index) => (
                <div key={index} className="w-full space-y-1">
                  ...
                  {row.email && !isValidEmail(row.email) && (
                    <p className="text-xs text-red-500 ml-2">Invalid email address</p>
                  )}
                </div>
              ))}
                ...
                {filtered.map((u) => (
                  ...
                  >
                    <div onClick={(e) => e.stopPropagation()}>
                      <Checkbox
                        checked={selected.includes(u.id) || u.alreadyAdded}
                    ...
                    <Avatar className="h-10 w-10">
                      ...
                      <AvatarFallback className="bg-blue-100 text-blue-600">{u.initials}</AvatarFallback>
                    </Avatar>
