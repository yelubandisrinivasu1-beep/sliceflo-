// components/list-view/customFields/Website.tsx
...
interface WebsiteProps {
  ...
  onCancel: () => void;
  initialData?: { name: string; description?: string; hyperlink?: boolean };
}
...
export function Website({
  ...
  const [hyperlink, setHyperlink] = useState(initialData?.hyperlink ?? false);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4 min-h-0">
        ...
        {/* Enable Hyperlink Toggle */}
        <div className="flex items-center justify-between py-1">
          <label htmlFor="hyperlink" className="text-xs text-muted-foreground cursor-pointer">
            Enable hyperlink
          </label>
          <Switch
            id="hyperlink"
            checked={hyperlink}
            onCheckedChange={setHyperlink}
          />
        </div>
