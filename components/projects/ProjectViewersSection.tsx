// components/projects/ProjectViewersSection.tsx
...
interface ProjectViewersSectionProps {
  projectId: string;
...
const ProjectViewersSection: React.FC<ProjectViewersSectionProps> = ({
  projectId,
  viewers = [],
  onAddViewers,
  onRemoveViewers,
  ...
  const [showAddInterface, setShowAddInterface] = useState(false); // New state for toggle
  ...
  console.log("ProjectViewersSection render", { teams });
    ...
    if (team?.teamMembers) {
      setSelectedTeamMembers(prev => ({
        ...
        [teamId]: checked
          ...
          : new Set(), // deselect all if unchecked
      }));
    }
  ...
  if (showAddInterface) {
    return (
      <div className="space-y-2">
        ...
        {/* // Tabs only show when showAddInterface=true OR viewers.length===0 but user wants to add */}
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
          <div className="flex items-center">
            {/* Back button — only when Add was clicked (viewers exist but showAddInterface=true) */}
            {showAddInterface && (
              <button
                ...
                filteredTeams.map((team) => {
                  ...
                  return (
                    <div key={team.id} className="rounded-md border border-transparent hover:border-accent">
                      ...
                      {/* TEAM ROW */}
                      <div
                        className="flex items-center justify-between py-1 px-1.5 hover:bg-accent/50 cursor-pointer"
                        ...
                        <div className="pl-8 pr-1 pb-1 space-y-1">
                          {team.teamMembers?.length > 0 ? (
                            <>
                              ...
                              <Button
                                size="sm"
                                className="mt-0 w-full bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 h-8 text-xs"
                                disabled={(selectedTeamMembers[team.id]?.size ?? 0) === 0 || isLoading || addedTeams.has(team.id)}
                                onClick={async () => {
                                  console.log("Selected members for team", team.id, selectedTeamMembers[team.id]);
                                  const selectedIds = Array.from(selectedTeamMembersRef.current[team.id] ?? []);
                                  if (selectedIds.length === 0) {
                                    console.log("No members selected for team", team.id);
                                    toast('error', { title: "Select at least one member" });
                                    return;
                                  }

                                  const newViewerIds = selectedIds.filter(id => !viewers.includes(id));
                                  console.log("New viewer IDs to add for team", team.id, newViewerIds);
                                  if (newViewerIds.length === 0) {
                                    console.log("All selected members are already viewers for team", team.id);
                                  ...
                                  setIsLoading(true);
                                  try {
                                    console.log("Adding team members as viewers:", newViewerIds);
                                    await onAddViewers(newViewerIds);
  ...
  // Get first 4 viewers for avatar display
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        {viewerDetails.length > 0 && (
          <div className="flex items-center -space-x-2">
            ...
            {remainingCount > 0 && (
              <div className="h-7 w-7 rounded-full bg-muted border-2 border-background ring-1 ring-border flex items-center justify-center">
      ...
      <ConfirmationModal
        ...
        title="Remove Viewer"
        confirmLabel="Remove"
        description="This will remove the viewer from the project. They will no longer be able to view it."
...
export default ProjectViewersSection;
