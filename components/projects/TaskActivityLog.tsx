...
export function TaskActivityLog({ taskId, projectId }: TaskActivityLogProps) {
  ...
  const fetchTaskActivityLogs = useActivityLogStore(
    (s) => s.fetchTaskActivityLogs
  )
  ...
  useEffect(() => {
    if (!taskId) return
    fetchTaskActivityLogs(taskId)
  }, [taskId, fetchTaskActivityLogs])
